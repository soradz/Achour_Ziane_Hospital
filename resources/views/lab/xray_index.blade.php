@extends('layouts.app')
@section('title', 'طلب تصوير إشعاعي')
@section('content')
<style>
...
<div class="main">
    <div class="page-header">
        ...
        <div class="ph-actions">
            <a href="{{ route('doctor.xray.results.list', ['back' => url()->current()]) }}" class="btn-nav btn-results">قائمة النتائج</a>
            <a href="{{ $patient->section === 'surgical' ? route('lab.two') : route('lab.one') }}" class="btn-nav btn-back">رجوع للمكتب</a>
        </div>
    </div>
    ...
    <div class="patient-bar">
        ...
        <div class="pb-info">
            <div class="pb-name">{{ $patient->first_name }} {{ $patient->last_name }}</div>
            <div class="pb-meta">العمر: {{ $patient->age }} سنة &nbsp;|&nbsp; {{ $patient->gender }} &nbsp;|&nbsp; الطبيب: {{ $patient->doctor }} &nbsp;|&nbsp; {{ $patient->section }}</div>
        </div>
        <div class="pb-id">PT-{{ str_pad($patient->id, 5, '0', STR_PAD_LEFT) }}</div>
    </div>

    <form method="POST" action="{{ route('xray.send.request', $patient->id) }}" id="xrayForm">
        @csrf
        <input type="hidden" name="body_part" id="bodyPartInput">
        <input type="hidden" name="urgency" id="urgencyInput" value="عادي">
        <input type="hidden" name="doctor_name" value="{{ session('name') }}">
