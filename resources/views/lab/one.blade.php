...
<div class="main">
    ...
    <div class="page-header">
        ...
        <div class="ph-info">
            <div class="ph-title">مكتب <span>الفحوصات 1</span></div>
            <div class="ph-sub">أهلاً، {{ session('name') }} — LAB OFFICE 01</div>
        </div>
        <div class="ph-right">
            @if($patients->where('lab_office', '!=', 1)->whereNull('received_at')->count() > 0)
            <a href="{{ route('patients.remaining') }}" class="alert-badge">
    ...
    @php
        $currentPatient = $patients->where('current_lab1', 1)->first();
    ...
    <div class="patient-card">
        ...
        <div class="info-grid">
            ...
            <div class="info-item full">
                ...
                <div class="info-value">
                    ...
                    {{ $currentPatient->created_at->format('Y-m-d') }}
                    &nbsp;
                    <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                    {{ $currentPatient->created_at->format('H:i') }}
                </div>
            </div>
        </div>

        <div class="receive-wrap">
            @if(!$currentPatient->received)
            <form method="POST" action="{{ route('patients.receive', $currentPatient->id) }}">
                @csrf
        ...
        <div class="action-row">
            <a href="{{ route('doctor.patient.show', $currentPatient->id) }}" class="act-btn btn-lab">
                <div class="act-icon">
                    <svg viewBox="0 0 24 24"><path d="M9 2v6l-2 4v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-8l-2-4V2"/><line x1="3" y1="12" x2="21" y2="12"/></svg>
                </div>
            ...
            <a href="{{ route('xray.index', $currentPatient->id) }}" class="act-btn btn-xray">
                <div class="act-icon">
        ...
        @if($lastLabRequest && $lastLabRequest->status === 'done')
        <a href="{{ route('doctor.results.show', $lastLabRequest->id) }}" class="btn-results-full">
        ...
        @elseif($lastLabRequest && $lastLabRequest->status === 'pending')
        ...
        <a href="{{ route('doctor.results.list') }}" class="btn-results-full blue-v">
    ...
    <div class="empty-state">
        ...
        <div class="empty-sub">في انتظار إحالة مريض إلى مكتب الفحوصات 1</div>
    </div>
    @endif
