...
<style>
    ...
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    @php $isSurgical = $patient->section !== 'medical'; @endphp
...
<div class="main" id="mainWrap">
    ...
    <div class="page-header">
        <div class="ph-logo">
            <div class="ph-logo-in">{{ $isSurgical ? '🔪' : '🏥' }}</div>
        </div>
        <div class="ph-info">
            <div class="ph-title">تعديل بيانات — <span>{{ $isSurgical ? 'القسم الجراحي' : 'القسم الطبي' }}</span></div>
            <div class="ph-sub">مستشفى عاشور زيان — أولاد جلال</div>
        </div>
        <div class="ph-btns">
            <a href="{{ route('patients.today', $patient->section) }}" class="btn-grey">📋 قائمة المرضى</a>
            <a href="{{ route('triage') }}" class="btn-cyan">↩ الفرز</a>
        </div>
    </div>
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('patients.update', $patient->id) }}">
            @csrf
            @method('PUT')
            <input type="hidden" name="section" value="{{ $patient->section }}">

            <!-- الاسم واللقب -->
            <div class="row">
                <div class="field">
                    <div class="field-label"><span class="fl-ic">👤</span> الاسم</div>
                    <input type="text" name="first_name" class="field-input" value="{{ $patient->first_name }}" required>
                </div>
                <div class="field">
                    <div class="field-label"><span class="fl-ic">👤</span> اللقب</div>
                    <input type="text" name="last_name" class="field-input" value="{{ $patient->last_name }}" required>
                </div>
            </div>
            ...
            <div class="row">
                <div class="field">
                    ...
                    <input type="number" name="age" class="field-input" value="{{ $patient->age }}">
                </div>
                <div class="field">
            ...
            <div class="row">
                <div class="field">
                    <div class="field-label"><span class="fl-ic">{{ $isSurgical ? '🔪' : '🩺' }}</span> الحالة</div>
                    <select name="status_select" id="statusSelect" class="field-input">
                        <option value="">— اختر الحالة —</option>
                        @foreach($allStatuses as $s)
                        <option value="{{ $s }}" {{ $patient->status==$s ? 'selected':'' }}>{{ $s }}</option>
                        @endforeach
                        <option value="أخرى" {{ $isCustom ? 'selected':'' }}>✏️ أخرى (اكتب)</option>
                    </select>
                </div>
                <div class="field">
                    <div class="field-label"><span class="fl-ic">👨‍⚕️</span> الطبيب</div>
                    <input type="text" name="doctor" class="field-input" value="{{ $patient->doctor }}" required>
                </div>
            </div>

            <!-- Custom status -->
            <div class="custom-wrap {{ $isCustom ? '' : 'hidden' }}" id="customWrap">
                <div class="field" style="margin-bottom:12px;">
                    ...
                    <input type="text" name="status" id="statusInput" class="field-input"
                           ...
                           value="{{ $isCustom ? $patient->status : '' }}">
                </div>
            </div>

            <!-- hidden status fallback -->
            <input type="hidden" name="status_fallback" id="statusFallback" value="{{ !$isCustom ? $patient->status : '' }}">

            <button type="submit" class="btn-submit">💾 حفظ التعديلات</button>

        </form>

        <a href="{{ route('patients.today', $patient->section) }}" class="back-link">↩ رجوع لقائمة المرضى</a>
    </div>
