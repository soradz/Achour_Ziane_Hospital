...
<div class="pg">
    ...
    <div class="pg-header">
        <div class="hdr-top">
            <a href="{{ $patient->section === 'surgical' ? route('lab.two') : route('lab.one') }}" class="back-btn">
    ...
    <form method="POST" action="{{ route('doctor.send.to.lab', $patient->id) }}" id="labForm">
        @csrf
        ...
        <div class="sec">
            ...
            <div class="tests-grid">
                ...
                <label class="test-item" onclick="toggleTest(this)">
                    <input type="checkbox" name="tests[]" value="{{ $t['name'] }}">
                    <div class="test-check">
                        <svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
                    </div>
                    <div class="test-ic">
                        <svg viewBox="0 0 24 24">{!! $svgs[$t['icon']] ?? $svgs['activity'] !!}</svg>
                    </div>
                    <div>
                        <div class="test-name">{{ $t['name'] }}</div>
                        <div class="test-cat">{{ $t['cat'] }}</div>
                    </div>
                </label>
                @endforeach
            </div>
