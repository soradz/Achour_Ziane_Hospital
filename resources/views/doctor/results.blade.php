...
<div class="pg no-print">
    <div class="pg-header">
        <div class="hdr-top">
            <a href="{{ $labRequest->patient->section === 'surgical' ? route('lab.two') : route('lab.one') }}" class="back-btn">← رجوع</a>
            @if($labRequest->status === 'done')
            <button onclick="window.print()" class="print-btn">🖨️ طباعة</button>
            @endif
            <div class="hdr-titles">
                <div class="hdr-title">نتائج التحاليل</div>
                <div class="hdr-sub">{{ $labRequest->patient->first_name }} {{ $labRequest->patient->last_name }}</div>
            </div>
        </div>
        <div class="pi-badges">
            <div class="pi-badge">👤 {{ $labRequest->patient->first_name }} {{ $labRequest->patient->last_name }}</div>
            <div class="pi-badge">🆔 {{ $labRequest->patient->id }}</div>
            <div class="pi-badge">👨‍⚕️ {{ $labRequest->doctor_name }}</div>
            <div class="pi-badge">🕐 {{ $labRequest->created_at->format('H:i') }}</div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="info-grid">
            <div class="info-item"><div class="info-label">درجة الاستعجال</div><div class="info-value"><span class="urg-badge urg-{{ $labRequest->urgency }}">{{ $labRequest->urgency }}</span></div></div>
            <div class="info-item"><div class="info-label">الحالة</div><div class="info-value">{{ $labRequest->status === 'done' ? '✅ مكتمل' : '⏳ في الانتظار' }}</div></div>
            <div class="info-item"><div class="info-label">تاريخ الطلب</div><div class="info-value">📅 {{ $labRequest->created_at->format('Y-m-d H:i') }}</div></div>
            <div class="info-item"><div class="info-label">عدد التحاليل</div><div class="info-value">🧪 {{ count($requestedTests) }} تحليل</div></div>
        </div>
    </div>

    {{-- نتائج مجمعة حسب التحليل --}}
    @if($labRequest->status === 'done' && !empty($results))
    @php
    ...
    <div class="card">
        ...
        @foreach($grouped as $testName => $indicators)
        <div class="analysis-block">
            <div class="ab-header">
                <span style="font-size:18px;">🧬</span>
                <span class="ab-name">{{ $testName }}</span>
                <span style="font-size:10px;color:var(--soft);">{{ count($indicators) }} مؤشر</span>
            </div>
            <table class="ind-table">
                ...
                <tbody>
                    @foreach($indicators as $indName => $indVal)
                    <tr>
                        <td class="ind-name">{{ $indName }}</td>
                        <td class="ind-val">{{ $indVal ?: '—' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <div class="card">
        ...
        <div class="no-results">
            @if($labRequest->status === 'done') ⚠️ لا توجد نتائج مسجلة
            @else ⏳ لم يتم إرسال النتائج بعد @endif
        </div>
    </div>
    @endif

    @if($labRequest->lab_notes)
    <div class="card"><div class="card-title"><div class="ic">📝</div> ملاحظات المخبر</div><div class="notes-box">{{ $labRequest->lab_notes }}</div></div>
    @endif
    @if($labRequest->notes)
    <div class="card"><div class="card-title"><div class="ic">💬</div> ملاحظات الطبيب</div><div class="notes-box">{{ $labRequest->notes }}</div></div>
    @endif
</div>
...
<div class="print-page">
    <div class="ps-header">
        ...
        <div style="text-align:left;">
            ...
            <div class="ps-date">{{ $labRequest->created_at->format('Y/m/d') }} — {{ $labRequest->created_at->format('H:i') }}</div>
        </div>
    </div>

    <div class="ps-patient-box">
        <div class="ps-pf"><div class="ps-pf-label">اسم المريض</div><div class="ps-pf-val">{{ $labRequest->patient->first_name }} {{ $labRequest->patient->last_name }}</div></div>
        <div class="ps-pf"><div class="ps-pf-label">رقم الملف</div><div class="ps-pf-val">{{ str_pad($labRequest->patient->id, 6, '0', STR_PAD_LEFT) }}</div></div>
        <div class="ps-pf"><div class="ps-pf-label">العمر / الجنس</div><div class="ps-pf-val">{{ $labRequest->patient->age ?? '—' }} / {{ $labRequest->patient->gender ?? '—' }}</div></div>
        <div class="ps-pf"><div class="ps-pf-label">الطبيب المحوِّل</div><div class="ps-pf-val">د. {{ $labRequest->doctor_name }}</div></div>
        <div class="ps-pf"><div class="ps-pf-label">درجة الاستعجال</div><div class="ps-pf-val">{{ $labRequest->urgency }}</div></div>
        <div class="ps-pf"><div class="ps-pf-label">رقم الطلب</div><div class="ps-pf-val">LB-{{ str_pad($labRequest->id, 5, '0', STR_PAD_LEFT) }}</div></div>
    </div>
    ...
    @foreach($grouped as $testName => $indicators)
    <div class="ps-section-title">{{ $testName }}</div>
    <table class="ps-table">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $indName }}</td>
                <td class="val-cell">{{ $indVal ?: '—' }}</td>
                <td style="color:#666;font-size:10px;">{{ $refRanges[$indName] ?? '—' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="ps-page-footer">
        ...
        <span>LB-{{ str_pad($labRequest->id, 5, '0', STR_PAD_LEFT) }} | {{ $labRequest->created_at->format('Y/m/d') }}</span>
    </div>
</div>
</div>
@endsection