...
<div class="main">
    ...
    <div class="section-label">⚡ نتائج اليوم ({{ $todayRequests->count() }})<div class="ln"></div></div>
    <div class="results-grid" style="margin-bottom:20px;">
        @foreach($todayRequests->values() as $i => $req)
        @php
        ...
        @endphp
        <a href="{{ route('doctor.results.show', $req->id) }}" class="result-card {{ $urgCls }}" style="animation-delay:{{ $i * 0.05 }}s">
            <div class="rc-avatar">🧑‍⚕️</div>
            <div class="rc-info">
                <div class="rc-name">{{ $req->patient->first_name }} {{ $req->patient->last_name }}</div>
                <div class="rc-meta">
                    👨‍⚕️ {{ $req->doctor_name }} &nbsp;|&nbsp;
    ...
    <div class="section-label">📁 نتائج سابقة ({{ $otherRequests->count() }})<div class="ln"></div></div>
    <div class="results-grid">
        @foreach($otherRequests->values() as $i => $req)
        @php
            $tests = is_array($req->requested_tests) ? $req->requested_tests : json_decode($req->requested_tests, true) ?? [];
            $urgCls = 'urg-' . ($req->urgency ?? 'عادي');
            $badgeCls = $req->urgency === 'حرج' ? 'badge-red' : ($req->urgency === 'مستعجل' ? 'badge-amber' : 'badge-green');
        @endphp
        <a href="{{ route('doctor.results.show', $req->id) }}" class="result-card {{ $urgCls }}" style="animation-delay:{{ $i * 0.04 }}s;opacity:0.85">
