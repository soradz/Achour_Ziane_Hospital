...
<div class="main">
    <div class="page-header">
        ...
        <a href="{{ route('admin.dashboard') }}" class="back-btn">↩ لوحة الإدارة</a>
    </div>
    ...
    <div class="ann-list">
        ...
        @endphp
        <div class="ann-card {{ $ann->is_active ? '' : 'inactive' }}" style="animation-delay:{{ $i * 0.05 }}s">
            <div class="ann-status {{ $ann->is_active ? 'status-on' : 'status-off' }}"></div>
            <div class="ann-body">
                <div class="ann-text">{{ $ann->text }}</div>
                <div class="ann-meta">
                    <span class="ann-cat {{ $catClass }}">{{ $catLabel }}</span>
                    <span class="ann-date">🕐 {{ $ann->created_at->format('Y-m-d H:i') }}</span>
                    @if($ann->is_active)
            ...
            <div class="ann-actions">
                ...
                <form method="POST" action="{{ route('admin.announcements.toggle', $ann->id) }}">
                    @csrf
                    <button type="submit" class="btn-icon {{ $ann->is_active ? 'btn-toggle-on' : 'btn-toggle-off' }}" title="{{ $ann->is_active ? 'إيقاف' : 'تفعيل' }}">
                        {{ $ann->is_active ? '✅' : '⏸' }}
                    </button>
                </form>
                {{-- تعديل --}}
                <button onclick="openEdit({{ $ann->id }}, '{{ addslashes($ann->text) }}')" class="btn-icon btn-edit" title="تعديل">✏️</button>
                {{-- حذف --}}
                <form method="POST" action="{{ route('admin.announcements.destroy', $ann->id) }}" onsubmit="return confirm('حذف هذا الإعلان؟')">
                    @csrf
                    @method('DELETE')
