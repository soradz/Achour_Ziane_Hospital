        ...
        <div class="sc sc-b" style="animation-delay:.08s">
            <div class="sc-top">
                <div>
                    ...
                    <div class="num">{{ $todayMedicalCount ?? 0 }}</div>
            ...
            <div class="bar"><div class="bar-f" style="--w:{{ min(($todayMedicalCount??0)*10,100) }}%"></div></div>
        </div>
        ...
        <div class="sc sc-r" style="animation-delay:.13s">
            <div class="sc-top">
                <div>
                    ...
                    <div class="num">{{ $todaySurgicalCount ?? 0 }}</div>
                    <div class="hint">مريض اليوم</div>
                </div>
                <div class="sc-icon">
            ...
            <div class="bar"><div class="bar-f" style="--w:{{ min(($todaySurgicalCount??0)*10,100) }}%"></div></div>
        </div>
        ...
        <div class="sc sc-g" style="animation-delay:.18s">
            <div class="sc-top">
                <div>
                    ...
                    <div class="num">{{ ($todayMedicalCount??0)+($todaySurgicalCount??0) }}</div>
            ...
            <div class="bar"><div class="bar-f" style="--w:{{ min((($todayMedicalCount??0)+($todaySurgicalCount??0))*5,100) }}%"></div></div>
        </div>
        ...
        <div class="sc sc-c" style="animation-delay:.23s">
            <div class="sc-top">
                <div>
                    ...
                    <div class="num">{{ $latestPatients->count() }}</div>
            ...
            <div class="bar"><div class="bar-f" style="--w:{{ min($latestPatients->count()*20,100) }}%"></div></div>
        </div>
    </div>
                    ...
                    <tr>
                        <td>
                            <div class="td-n">
                                ...
                                <span class="td-nm">{{ trim(($patient->first_name??'').' '.($patient->last_name??'')) ?: '—' }}</span>
                            </div>
                        </td>
                        <td><span class="td-age">{{ !empty($patient->age) ? $patient->age : '—' }}</span></td>
                        <td>
                            @if(!empty($patient->gender))
                            <span class="gender-badge {{ $patient->gender==='أنثى' ? 'gb-f' : 'gb-m' }}">
                                {{ $patient->gender }}
                            </span>
                        ...
                        <td>
                            @if(!empty($patient->status))
                                @php $s=$patient->status; @endphp
                                @if(str_contains($s,'حرج')||str_contains($s,'خطر'))
                                    <span class="pill pl-r"><div class="pd"></div>{{ $s }}</span>
                                @elseif(str_contains($s,'مستقر')||str_contains($s,'جيد'))
                                    <span class="pill pl-g"><div class="pd"></div>{{ $s }}</span>
                                @elseif(str_contains($s,'متوسط'))
                                    <span class="pill pl-a"><div class="pd"></div>{{ $s }}</span>
                                @else
                                    <span class="pill pl-b"><div class="pd"></div>{{ $s }}</span>
                                @endif
                            @else <span style="color:var(--muted)">—</span>
                            @endif
                        </td>
                        <td>
                            @if(!empty($patient->section))
                                @php $sec=$patient->section; @endphp
                                @if(str_contains($sec,'طبي')||str_contains($sec,'داخلي')||$sec==='medical')
                                    <span class="stag st-b">
                                        <svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>
                                        {{ $sec }}
                                    </span>
                                @elseif(str_contains($sec,'جراح')||$sec==='surgical')
                                    <span class="stag st-r">
                                        <svg viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                                        {{ $sec }}
                                    </span>
                                @else
                                    <span class="stag st-d">{{ $sec }}</span>
                                @endif
                            @else <span style="color:var(--muted)">—</span>
                            @endif
                        </td>
                        <td>
                            <div class="td-dt">
                                ...
                                {{ $patient->created_at->format('d/m/Y') }}<br>
                                <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
        ...
        <div class="ann-hdr">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
                <span class="ann-cnt">
                    ...
                    @php $annTotal = isset($announcements) ? $announcements->count() : 0; @endphp
                    {{ $annTotal }} إعلان
                </span>
                <a href="{{ route('admin.announcements.index') }}" class="btn-ann-full">
        ...
        <div class="ann-wrap">
            ...
            <div class="ann-list">
                ...
                @if(isset($announcements) && $announcements->count() > 0)
                    ...
                    @php $cat=$catMap[$ann->category??'gen']??$catMap['gen']; @endphp
                    <div style="animation-delay:{{ 0.05+$i*0.04 }}s">
                        <div class="ann-row">
                            <form method="POST" action="{{ route('admin.announcements.toggle', $ann->id) }}" class="tg-f">
                                @csrf
                                <button type="submit" class="tg-btn {{ $ann->is_active ? 'on' : 'off' }}" title="{{ $ann->is_active ? 'إيقاف' : 'تفعيل' }}"></button>
                            </form>
                            <span class="cb {{ $cat[0] }}">{{ $cat[1] }}</span>
                            <div class="ann-text {{ $ann->is_active ? '' : 'muted' }}">{{ $ann->text }}</div>
                            <div class="ann-btns">
                                <button class="abtn ab-e" title="تعديل" onclick="openEdit({{ $ann->id }}, `{{ addslashes($ann->text) }}`)">
                                    <svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                </button>
                                <button class="abtn ab-d" title="حذف" onclick="openDel({{ $ann->id }}, `{{ addslashes($ann->text) }}`)">
                                    <svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>
                                </button>
                            </div>
                        </div>
                        <div class="ann-edit" id="ae-{{ $ann->id }}">
                            <form method="POST" action="{{ route('admin.announcements.update', $ann->id) }}">
                                @csrf @method('PUT')
                                <input type="text" name="text" id="aein-{{ $ann->id }}" class="ae-in" maxlength="300" required>
                                <button type="submit" class="ae-sv">حفظ</button>
                                <button type="button" class="ae-cl" onclick="closeEdit({{ $ann->id }})">إلغاء</button>
                            </form>
                        </div>
                    </div>
                    @endforeach
...
function openDel(id,text){
    ...
    document.getElementById('delForm').action=`{{ url('admin/announcements') }}/${id}`;
