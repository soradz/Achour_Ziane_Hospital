...
<div class="main">
    ...
    <div class="page-header">
        ...
        <a href="{{ session('xray_results_back', route('home')) }}" class="back-btn">
    ...
    <div class="section-lbl sl-done status-section" data-status="done">
        ...
        <span class="sl-count">{{ $doneReqs->count() }}</span>
        <div class="sl-line"></div>
    </div>
    @foreach($doneReqs->values() as $i => $req)
    @php $urgCls='rc-urg-'.($req->urgency??'عادي'); $bc=$req->urgency==='حرج'?'badge-red':($req->urgency==='مستعجل'?'badge-amber':'badge-green'); @endphp
    <a href="{{ route('doctor.xray.results.show', $req->id) }}"
       class="result-card done {{ $urgCls }} status-done"
       data-name="{{ strtolower($req->patient->first_name.' '.$req->patient->last_name) }}"
       style="animation-delay:{{ $i*.04 }}s">
        <div class="rc-status-icon si-done">
            <svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
        </div>
        <div class="rc-info">
            <div class="rc-patient">{{ $req->patient->first_name }} {{ $req->patient->last_name }}</div>
            <div class="rc-part">{{ $req->body_part }}</div>
            <div class="rc-meta">
                <span><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>{{ $req->doctor_name }}</span>
                <span><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>{{ $req->updated_at->format('Y-m-d H:i') }}</span>
            </div>
        </div>
        <div class="rc-right">
    ...
    <div class="section-lbl sl-pending status-section" data-status="pending" style="margin-top:18px;">
        ...
        <span class="sl-count">{{ $pendingReqs->count() }}</span>
        <div class="sl-line"></div>
    </div>
    @foreach($pendingReqs->values() as $i => $req)
    @php $urgCls='rc-urg-'.($req->urgency??'عادي'); $bc=$req->urgency==='حرج'?'badge-red':($req->urgency==='مستعجل'?'badge-amber':'badge-green'); @endphp
    <div class="result-card {{ $urgCls }} status-pending"
         data-name="{{ strtolower($req->patient->first_name.' '.$req->patient->last_name) }}"
         style="animation-delay:{{ $i*.04 }}s">
        ...
        <div class="rc-info">
            ...
            <div class="rc-meta">
                <span><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>{{ $req->doctor_name }}</span>
                <span><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>{{ $req->created_at->format('Y-m-d H:i') }}</span>
            </div>
        </div>
        <div class="rc-right">
