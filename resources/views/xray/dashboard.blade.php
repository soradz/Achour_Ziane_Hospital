@extends('layouts.app')
@section('title', 'لوحة التصوير الإشعاعي')
@section('content')
<style>
...
<div class="main">
    ...
    <div class="page-header">
        ...
        <div class="ph-info">
            ...
            <div class="ph-sub">مرحباً {{ session('name') }} — XRAY WORKER DASHBOARD</div>
        </div>
        <div class="ph-right">
    ...
    <div class="stats-row">
        <div class="stat-card sc-total" style="animation-delay:.05s">
            ...
            <div><div class="stat-num sn-blue">{{ $requests->count() }}</div><div class="stat-lbl">إجمالي الطلبات</div></div>
        </div>
        <div class="stat-card sc-pending" style="animation-delay:.1s">
            ...
            <div><div class="stat-num sn-amber">{{ $pending }}</div><div class="stat-lbl">في الانتظار</div></div>
        </div>
        <div class="stat-card sc-done" style="animation-delay:.15s">
            ...
            <div><div class="stat-num sn-green">{{ $done }}</div><div class="stat-lbl">مكتملة</div></div>
        </div>
    </div>
    ...
    <div class="sec-header">
        ...
        <span class="sec-badge sb-amber">{{ $pendingReqs->count() }}</span>
    ...
    @else
    @foreach($pendingReqs->values() as $i => $req)
    @php $bc = $req->urgency==='حرج'?'badge-red':($req->urgency==='مستعجل'?'badge-amber':'badge-green'); @endphp
    <div class="req-card urg-{{ $req->urgency }}" id="row-{{ $req->id }}" style="animation-delay:{{ $i*.05 }}s">
        <div class="req-header" onclick="toggleRow({{ $req->id }})">
            <div class="req-num">{{ str_pad($i+1,2,'0',STR_PAD_LEFT) }}</div>
            <div class="req-info">
                <div class="req-name">{{ $req->patient->first_name ?? '—' }} {{ $req->patient->last_name ?? '' }}</div>
                <div class="req-tags">
                    <span class="req-part-tag">
                        <svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/></svg>
                        {{ $req->body_part }}{{ $req->side ? ' ('.$req->side.')' : '' }}
                    </span>
            ...
            <div style="display:flex;gap:8px;align-items:center;">
                <span class="badge {{ $bc }}">{{ $req->urgency }}</span>
        ...
        <div class="req-panel">
            ...
            <div class="notes-box">
                ...
                ملاحظة الطبيب: {{ $req->notes }}
            </div>
            @endif
            <form method="POST" action="{{ route('xray.results.store', $req->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="field-label">UPLOAD IMAGE — رفع صورة الأشعة</div>
                <label class="upload-area" id="upload-{{ $req->id }}">
                    <input type="file" name="image" accept="image/*,.pdf" onchange="showFile(this,{{ $req->id }})" required>
                    <div class="upload-icon-wrap">
                        <svg viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                    </div>
                    <div class="upload-text">اضغط لرفع صورة الأشعة</div>
                    <div class="upload-sub">JPG · PNG · PDF</div>
                    <div class="file-preview" id="file-{{ $req->id }}"></div>
                </label>
    ...
    <div class="sec-header" style="margin-top:10px;">
        ...
        <span class="sec-badge sb-green">{{ $doneReqs->count() }}</span>
    ...
    @else
    @foreach($doneReqs->values() as $i => $req)
    <div class="done-row" style="animation-delay:{{ $i*.04 }}s">
        <div class="done-status"><svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg></div>
        <div class="done-info">
            <div class="done-name">{{ $req->patient->first_name ?? '—' }} {{ $req->patient->last_name ?? '' }}</div>
            <div class="done-meta">
                <span><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/></svg>{{ $req->body_part }}{{ $req->side ? ' ('.$req->side.')' : '' }}</span>
                <span><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>{{ $req->updated_at->format('H:i') }}</span>
            </div>
        </div>
