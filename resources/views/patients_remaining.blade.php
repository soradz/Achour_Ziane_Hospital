@extends('layouts.app')
@section('title', 'المرضى المتبقين - مكتب الفحوصات 1')
@section('content')
<style>
...
.ph-logo-wrap::before{content:'';position:absolute;inset:-2px;border-radius:12px;background:conic-gradient(var(--blue-d),var(--blue),#93c5fd,var(--blue-d));animation:spin 4s linear infinite;z-index:0;}
...
.ph-logo svg{width:18px;height:18px;stroke:var(--blue-d);fill:none;stroke-width:1.8;}
.ph-info{flex:1;}
...
.table-title svg{width:15px;height:15px;stroke:var(--blue-d);fill:none;stroke-width:2;}
...
@for($i=1;$i<=15;$i++)
tbody tr:nth-child({{$i}}){animation-delay:{{0.12+$i*0.04}}s}
@endfor
...
.td-av svg{width:14px;height:14px;stroke:var(--blue-d);fill:none;stroke-width:1.8;}
...
<div class="main">
    ...
    <div class="page-header">
        ...
        <div class="ph-info">
            <div class="ph-title">المرضى المتبقين — <span>مكتب الفحوصات 1</span></div>
            <div class="ph-sub">REMAINING PATIENTS — LAB OFFICE 01</div>
        </div>
        <a href="{{ route('lab.one') }}" class="btn-back">
        ...
        <div class="table-scroll">
            <table>
                ...
                <tbody>
                    ...
                    @php $isSent = $patient->lab_office == 1; @endphp
                    <tr>
                        <td><span class="num-badge">{{ $index + 1 }}</span></td>
                        <td>
                            <div class="td-n">
                                <div class="td-av">
                                    <svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                                </div>
                                <span class="td-nm">{{ $patient->first_name }} {{ $patient->last_name }}</span>
                            </div>
                        </td>
                        <td><span class="td-age">{{ $patient->age ?? '—' }}</span></td>
                        <td>
                            @if($patient->gender === 'ذكر')
                            <span class="gender-badge gb-m">ذكر</span>
                            @elseif($patient->gender === 'أنثى')
                        ...
                        <td>
                            <span class="sec-pill {{ $patient->section === 'medical' ? 'sp-med' : 'sp-surg' }}">
                                @if($patient->section === 'medical')
                        ...
                        <td>
                            @if($patient->status)
                            <span class="st-pill">{{ $patient->status }}</span>
                            @else
                            <span style="color:var(--muted)">—</span>
                            @endif
                        </td>
                        ...
                        <td>
                            ...
                            @else
                            <form method="POST" action="{{ route('lab.sendPatient', $patient->id) }}">
                                @csrf
