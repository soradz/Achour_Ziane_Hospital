...
<div class="main">
    <div class="page-header">
        ...
        <div class="ph-info">
            ...
            <div class="ph-sub">أهلاً، {{ session('name') }}! — قائمة طلبات التحاليل</div>
        </div>
        <div class="ph-right">
            <div class="ph-chip"><div class="live-d"></div> نشط</div>
            <a href="{{ route('logout') }}" class="back-btn">↩ خروج</a>
        </div>
    </div>
    ...
    <div class="stats-row">
        <div class="stat-card" style="animation-delay:.05s"><div class="stat-num sn-blue">{{ $requests->count() }}</div><div class="stat-lbl">إجمالي اليوم</div></div>
        <div class="stat-card" style="animation-delay:.1s"><div class="stat-num sn-amber">{{ $pending }}</div><div class="stat-lbl">في الانتظار</div></div>
        <div class="stat-card" style="animation-delay:.15s"><div class="stat-num sn-green">{{ $done }}</div><div class="stat-lbl">مكتملة</div></div>
    </div>
    ...
    <div class="queue-list">
        @foreach($pendingRequests->values() as $i => $req)
        @php
        ...
        @endphp
        <div class="req-row {{ $urgClass }}" id="row-{{ $req->id }}">
            <div class="req-header" onclick="toggleRow({{ $req->id }})">
                ...
                <div class="req-info">
                    ...
                    <div class="req-meta">👨‍⚕️ {{ $req->doctor_name }} &nbsp;|&nbsp; 🕐 {{ $req->created_at->format('H:i') }} &nbsp;|&nbsp; 🧪 {{ count($tests) }} تحليل</div>
                </div>
                <div class="req-right">
                    <span class="badge {{ $badgeCls }}">{{ $req->urgency }}</span>
            ...
            <div class="req-panel">
                ...
                @if($req->notes)
                <div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:10px;padding:9px 13px;font-size:12px;color:#92400e;margin-bottom:16px;">📝 {{ $req->notes }}</div>
                @endif

                {{-- نموذج النتائج --}}
                <form method="POST" action="{{ route('lab.results.store', $req->id) }}">
                    @csrf

                    <div class="analysis-blocks">
                        @foreach($tests as $test)
                        @php $inds = $indicators[$test] ?? []; @endphp
                        <div class="analysis-block">
                            <div class="analysis-block-header">
                                <span class="ab-icon">🧬</span>
                                <span class="ab-name">{{ $test }}</span>
                                @if(count($inds) > 0)
                                <span class="ab-count">{{ count($inds) }} مؤشر</span>
                                @endif
                            </div>
                            @if(count($inds) > 0)
                            <div class="indicators-grid">
                                ...
                                <div class="indicator-cell">
                                    <div class="ind-top">
                                        ...
                                        <div class="ind-ref">{{ $ind['ref'] }}</div>
                                    </div>
                                    <input
                                        type="text"
                                        name="results[{{ $test }}][{{ $ind['name'] }}]"
                                        class="ind-input"
                            ...
                            <div style="padding:12px 16px;">
                                <input type="text" name="results[{{ $test }}]" class="ind-input" style="width:100%" placeholder="النتيجة...">
                            </div>
                            @endif
                        </div>
                        @endforeach
